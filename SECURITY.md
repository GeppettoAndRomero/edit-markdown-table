...
`edit-markdown-table` runs entirely in your browser. There is no server component and no
account system, so your table data is never uploaded. Most classic web vulnerabilities
...
dependencies, a service worker caching bug, or anything that could cause table data to
leave your device.
...
- The table parser/serializer, the service worker, and the PWA manifest.
- Anything that could send table data off the device.
